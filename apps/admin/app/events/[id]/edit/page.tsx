...
export default async function EditEventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const existingImages = Array.isArray(event.images) ? (event.images as string[]) : []
  ...
  return (
    <div className="p-8 max-w-2xl space-y-6">
      ...
      <h1 className="font-display text-3xl text-text-high">Edit Event</h1>
      ...
      <form action={update} className="space-y-5">
        <div className="space-y-1.5">
          ...
          <input name="name" defaultValue={event.name} className={base} required />
        </div>
        ...
        <EventFormFields
          ...
          defaults={{
            ...
            property_id: event.property_id ?? undefined,
            type: event.type,
          }}
        />
        ...
        <div className="space-y-1.5">
          ...
          <textarea name="description" defaultValue={event.description ?? ''} rows={4} className={base} />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label className="text-xs text-text-low uppercase tracking-[0.1em] font-medium">Start Date & Time *</label>
            <input type="datetime-local" name="start_datetime" defaultValue={toDatetimeLocal(event.start_datetime)} className={base} required />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs text-text-low uppercase tracking-[0.1em] font-medium">End Date & Time *</label>
            <input type="datetime-local" name="end_datetime" defaultValue={toDatetimeLocal(event.end_datetime)} className={base} required />
          </div>
        </div>

        <div className="space-y-1.5">
          <label className="text-xs text-text-low uppercase tracking-[0.1em] font-medium">Event Photos</label>
          <ImageUploader name="images" defaultValue={existingImages} max={6} folder="events" />
        </div>
        ...
        <div className="flex items-center gap-3 p-4 bg-surface-dark rounded-xl border border-gold-border/20">
          ...
          <input
            ...
            name="is_approved"
            value="true"
            defaultChecked={event.is_approved}
